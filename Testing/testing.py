...
class test(un.TestCase):
    def testCase1(self):
        testSudoku = '087002010204017003006800705508001000640008100002050670439180007020900030700023091'
        sudokuBoard = ToBoard(testSudoku)
        result = SolveSudoku(sudokuBoard)
        sudokuTest = verifySudoku(testSudoku, result)
        self.assertEqual(True, sudokuTest,
                         msg=f"Error on sequence:{testSudoku}")
    ...
    def testSudokuFile(self):
        with open('testdata.txt', 'r') as S:
            for sudoku in S:
                ...
                self.assertTrue(solverresult == True,
                                msg='Error on Subsequence:{sudoku}')
